...
void LIS3MDL_TWI::calibrate() {
    ...
    int16_t uncalibratedValues[3];
	readXYZ(&uncalibratedValues[0],&uncalibratedValues[1],&uncalibratedValues[2]);
    uncalibratedValues[0] -= _bias[0];
    uncalibratedValues[1] -= _bias[1];
    uncalibratedValues[2] -= _bias[2];

    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
        result[i] += _calibrationMatrix[i][j] * float(uncalibratedValues[j]);
        }
    }
